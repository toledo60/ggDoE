...
gg_glm(glm_model, discrete_edm = FALSE)
```

There seems to be a non-linear trend in the 'Residuals vs Fitted' plot above. This means that the systematic component of the GLM can be improved. This would usually involve changing the link function, adding new predictor variables, or transforming the current predictors. The 'Working Responses vs Link Predictors' looks linear, so the chosen link function seems to be appropriate. One can log transform the predictor variables and recheck the graphs.

```{r glm-model-3,fig.height=6,fig.width=8}
glm_model_2 <- glm(Volume ~ log(Girth) + log(Height), family = Gamma(link = "log"), data = trees)

gg_glm(glm_model_2, discrete_edm = FALSE, standard_errors = TRUE)
```
...
gg_glm(glm_model_2, discrete_edm = FALSE, which_plots = 7:8)
```
