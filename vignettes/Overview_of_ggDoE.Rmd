...
7. [Interaction Effects Plot for a Factorial Design](#interaction-effects-plot-for-a-factorial-design)
8. [Main Effects Plot for a Factorial Design](#main-effects-plot-for-a-factorial-design)
9. [Contour Plots for Response Surface Methodology](#contour-plots-for-response-surface-methodology)
...
Below is an example from generating a random Latin hypercube design using the R package `lhs`

```{r,fig.height=6,fig.width=8}
set.seed(10)
random_LHS <- lhs::randomLHS(n=15, k=4)
pair_plots(random_LHS,n_columns=3,grid = TRUE)
```
...
pair_plots(maximin_LHS,n_columns=3,point_color = 'red')
```
